...
public:
    ...
    /*void WriteAtEfficiently (const char *str, int x, int y) {
        // draw background box for the text
        glColor3f (backgroundColor.r, backgroundColor.g, backgroundColor.b);
        
        int boxLength = GetStringLength (str)-1;
        float x1 = XToLCD(x);
        float y1 = YToLCD(y);
        float xSpace = XToLCD (charWidth);
        
        glRectf (x1, y1, x1 + (boxLength * xSpace), YToLCD(y+17));
        
        // draw text
        glColor3f (fontColor.r, fontColor.g, fontColor.b);
        y1 = YToLCD(y+10); // adjust this value as needed to better mimic how it looks on the proteus
        
        for (int i = 0; str[i] != '\0'; i++) {
            glRasterPos3f (x1 + i * xSpace, y1, 0.0);
    ...
    void SetFontColor (int color) {
        ...
        // cout << fontColor.r << ", " << fontColor.g << ", "  << fontColor.b << endl;
    }
